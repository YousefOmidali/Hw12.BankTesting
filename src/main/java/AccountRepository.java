...
public class AccountRepository {
    ...
    public Integer insert(Account account) throws SQLException {
        Integer id = 0;
        String insert = "insert into account (first_name, last_name, national_code,amount, status)" +
                " values (?,?,?,?,?);";
    ...
    public void update(Account account) throws SQLException {
        String update = "update Account set first_name = ?,last_name = ?, national_code = ?," +
                " amount = ?, branch = ? , status = ? where Id = ?";
    ...
    public Account findByNationalCode(Integer nationalCode) throws SQLException {
        ...
        if (resultSet.next()) {
            account = new Account(resultSet.getInt("amount"),
                    resultSet.getInt("Id"),
